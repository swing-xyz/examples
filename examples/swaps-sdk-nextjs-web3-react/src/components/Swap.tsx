...
const Swap = () => {
  ...
  const [resultLogs, setResultLogs] = useState<{time: string, log?: string, logType?: string}[] | null>([]);
  ...
  async function startTransfer() {
    ...
    const transferListener = swingSDK.on(
      ...
      async (transferStepStatus, transferResults) => {
        setResultLogs((prevItems) => [
          ...prevItems!, 
          { time: new Date().toISOString(), log: `Transaction Status -> ${transferStepStatus.status}`, logType: 'MESSAGE'}, 
          { time: new Date().toISOString(), log: `Transfer Step -> ${transferStepStatus.step}`, logType: 'MESSAGE'}, 
        ])
        ...
        switch (transferStepStatus.status) {
          case "ACTION_REQUIRED":
            setResultLogs((prevItems) => [
              ...prevItems!, 
              { time: new Date().toISOString(), log: `ACTION REQUIRED: Prompting MetaMask`, logType: 'ACTION_REQUIRED'}, 
            ])
            break;
          case "CHAIN_SWITCH_REQUIRED":
            setResultLogs((prevItems) => [
              ...prevItems!, 
              { time: new Date().toISOString(), log: `CHAIN SWITCH REQUIRED: Prompting MetaMask`, logType: 'CHAIN_SWITCH'}, 
            ])
    ...
    try {
      ...
      setResultLogs((prevItems) => [
        ...prevItems!, 
        { time: new Date().toISOString(), log: `Error -> ${(error as Error).message}`, logType: 'ERROR'}, 
      ])
    }
    ...
    <div className="flex flex-col gap-3 items-center justify-center">
      ...
      <div className="w-full flex border-2 rounded-xl border-zinc-100">
        <div className="w-full flex justify-between items-center p-1">
          ...
          >
            <img
              src={"https://raw.githubusercontent.com/polkaswitch/assets/master/blockchains/polygon/info/logo.png"}
              alt={""}
          ...
          <div
            className="flex mx-auto justify-center p-3 group"
          >
          ...
          >
            ...
            <img
              src={"https://s3.ap-northeast-1.amazonaws.com/platform.swing.xyz/assets/usdc/0c7e4c2dc978757682eed00e438afa3eab3a97bdb63ad57abfbb9d79eb42f006.png"}
              alt={""}
              className="rounded-full w-7 h-7"
            />
          </div>
      ...
      <div className="w-full">
        <Select onValueChange={(e) => setTransferRoute(routes?.filter(route => route.quote.integration === e)[0]!)}>
          <SelectTrigger className="w-[180px] bg-zinc-900 w-full">
            <SelectValue placeholder="Select Route" className="bg-zinc-900" />
          </SelectTrigger>
          <SelectContent className="bg-zinc-900">
            {routes?.map(route => (
              <SelectItem key={route.quote.integration} value={route.quote.integration} className="hover:text-zinc-900"
              >{route.quote.integration}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>
      ...
      {isConnected ? (
        ...
        <StatusSheet isOpen={!!status} logs={resultLogs!} onCancel={async () => {
