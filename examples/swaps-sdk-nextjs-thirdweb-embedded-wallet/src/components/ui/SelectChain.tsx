...
export const SelectChain: FC<ISelectChain> = ({ chains, onChainAndTokenSelected, toChain }) => {
        ...
        <div className="w-full">
            {selectedChain?.chain ? (
                    ...
                    <div className="h-[400px] scrollbar scrollbar-thumb-black overflow-y-auto p-3">
                        <div className="mt-10">
                            {tokens?.map((token: Token, index: number) => (
                                <SingleToken key={index} chain={selectedChain.chain!} token={token} onSelectChain={handleSelectedChain} />
                            ))}
                        </div>
                    </div>
                </div>
                ...
                <div className="h-[400px] mt-4 scrollbar scrollbar-thumb-black overflow-y-auto p-3">
                    {chains?.map((chain: Chain, index: number) => <SingleChain key={index} chain={chain} onSelectChain={handleSelectedChain} />)}
                </div>
            )}
        </div>
    );
