...
import SwingSDK, { TransferStepResults, TransferStepResult, TransferRoute, TransferParams, Chain, Token } from '@swing.xyz/sdk';
...
import { SelectChain } from './ui/SelectChain';
import {
    MdCheckCircle,
    MdLocalGasStation,
...
function calculateTokenAmount({ amount, decimals }: { amount: number; decimals: number }) {
...
const Swap = () => {
    ...
    const [fromTokenLocalAmount, setFromTokenLocalAmount] = useState(0);
    ...
    useEffect(() => {
        ...
        swing
            ...
            .then(async () => {
                ...
                let _sendChains = await swing.getAvailableSendChains({ type: 'swap' });
                // _sendChains = _sendChains.filter((chain: Chain) => allowedChains.includes(chain.slug));

                setSendChains(_sendChains);

                const _fromChain = _sendChains[Math.floor(Math.random() * _sendChains.length)];

                let _sendChainTokens = await swing.getAvailableSendTokens({ type: 'swap', fromChainSlug: _fromChain.slug });
                console.log('send', _sendChainTokens);
                const _fromToken = _sendChainTokens[Math.floor(Math.random() * _sendChainTokens.length)];

                setFromToken(_fromToken);
                setFromChain(_fromChain);

                let _receiveChains = await swing.getAvailableReceiveChains({
                    type: 'swap',
                ...
                const _toChain = _receiveChains[Math.floor(Math.random() * _receiveChains.length)];

                let _receiveChainsTokens = await swing.getAvailableReceiveTokens({
                    type: 'swap',
                ...
                const _toToken = _receiveChainsTokens[Math.floor(Math.random() * _receiveChainsTokens.length)];
    ...
    async function getTokenBalances() {
        if (toChain) {
            const _balance = await swingSDK.wallet.getBalance(toChain?.slug!, toToken?.symbol!, walletAddress);
            setToTokenBalance(_balance);
        }
        if (fromChain) {
            const _balance = await swingSDK.wallet.getBalance(fromChain?.slug!, fromToken?.symbol!, walletAddress);
    ...
    async function getQuote() {
        ...
        try {
            ...
            const bestQuote = _quotes.routes.sort((a, b) => Number(a.quote.amount) - Number(b.quote.amount))[0];
            const quoteIntegration = await swingSDK.getIntegration(bestQuote?.quote?.integration!);

            setToTokenLocalAmount(bestQuote?.quote?.amountUSD!);

            setQuotes(_quotes.routes);
            setTransferRoute({ ...bestQuote, ...quoteIntegration } as any);
        } catch (error: any) {
    ...
    async function startTransfer() {
        ...
        const transferListener = swingSDK.on('TRANSFER', async (transferStepStatus: any, transferResults: any) => {
    ...
    return (
        <div id="#altcoin" className="w-full max-w-4xl p-5 m-auto space-y-4 bg-white rounded-4xl lg:ml-auto md:w-[20rem] sm:w-[20rem] lg:w-[21rem]">
            <div className="flex flex-col space-y-3">
                <div className="w-full flex justify-between items-center">
                    ...
                    <div className="flex min-w-5 min-h-5 rounded-4xl bg-gray-100">
                        <div className="bg-black text-white rounded-4xl text-[9px] font-bold px-2 py-1.5">Swap</div>
                        <div className=" text-gray-500 rounded-4xl text-[9px] font-bold px-2 py-1.5">Stake</div>
                    </div>
                </div>
                ...
                <div className="w-full flex border-2 rounded-xl border-zinc-100">
                    <div className="w-full flex justify-between items-center p-1">
                        ...
                        >
                            <img src={fromChain?.logo} alt={fromChain?.name} className="rounded-full w-7 h-7" />
                            <div className="flex flex-col">
                                <h5 className="text-gray-400 font-bold text-[9px]">From</h5>
                                <h5 className="font-bold text-[10px] capitalize">{fromChain?.name}</h5>
                            </div>
                        </div>
                        <div
                            ...
                            onClick={() => {
                                ...
                                setFromChain(tempToChain as any);

                                const tempToToken = { ...toToken };
                                setToToken(fromToken);
                                setFromToken(tempToToken as any);
                        ...
                        >
                            <div className="flex flex-col">
                                <h5 className="text-gray-400 font-bold text-[9px] text-right">To</h5>
                                <h5 className="font-bold text-[10px] capitalize">{toChain?.name}</h5>
                            </div>
                            <img src={toChain?.logo} alt={toChain?.name} className="rounded-full w-7 h-7" />
                        </div>
                    </div>
                </div>

                <div className="w-full flex flex-col border-2 border-zinc-100 space-y-1 rounded-xl bg-zinc-950/[0.02] p-3">
                    <h4 className="w-full text-[11px] font-bold text-zinc-700">Deposit</h4>
                    <div className="flex justify-between items-center">
                        <input
                            ...
                            type="number"
                        />
                        <div className="flex items-center space-x-1 bg-white p-2 rounded-xl hover:cursor-pointer" onClick={openSendDialog}>
                            <img
                                src={fromToken?.logo ?? ''}
                                alt={fromToken?.symbol ?? defaultTransferParams.fromChain}
                                className="rounded-full w-4 h-4"
                            />
                            <p className="text-zinc-800 text-xs">{fromToken?.symbol ?? <>USDC</>}</p>
                            <MdOutlineArrowDropDown className="rounded-2xl w-5 h-5 text-zinc-300 font-bold" />
                        </div>
                    </div>

                    <div className="flex justify-between items-center">
                        <p className="text-xs p-0 m-0 text-zinc-950/[0.6]">{!transferRoute ? <>$0</> : <>{}</>}</p>
                        <p className="text-[10px] p-0 m-0 text-zinc-950/[0.6]">
                            {Number(fromTokenBalance).toFixed(3)} {fromToken?.symbol ?? <>USDC</>} available
                        </p>
                    </div>
                </div>

                <div className="w-full flex flex-col border-2 border-zinc-100 space-y-2 rounded-xl bg-zinc-950/[0.02] p-3">
                    <h4 className="w-full text-[11px] font-bold text-zinc-700">Receive</h4>
                    <div className="flex justify-between items-center">
                        <input
                            aria-label="receive"
                            disabled
                            ...
                            value={receiveAmount}
                        />
                        <div className="flex items-center space-x-1 bg-white p-2 rounded-xl hover:cursor-pointer" onClick={openReceiveDialog}>
                            <img
                                src={toToken?.logo ?? ''}
                                alt={toToken?.symbol ?? defaultTransferParams.fromChain}
                                className="rounded-full w-4 h-4"
                            />
                            <p className="text-zinc-800 text-xs">{toToken?.symbol ?? <>USDC</>}</p>
                            <MdOutlineArrowDropDown className="rounded-2xl w-5 h-5 text-zinc-300 font-bold" />
                        </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                        <p className="text-xs p-0 m-0 text-zinc-950/[0.6]">
                            {!transferRoute || receiveAmount === '0' ? <>$0</> : <>{formatUSD(toTokenLocalAmount)}</>}
                        </p>
                        <p className="text-[10px] p-0 m-0 text-zinc-950/[0.6]">
                    ...
                    <div className="flex justify-between items-center border-b my-0/!">
                        ...
                        <div className="flex items-center space-x-1 bg-white p-2 rounded-xl">
                            {!quotes.length ? (
                                ...
                                >
                                    <img
                                        src={(transferRoute as any)?.logo}
                                        alt={transferRoute?.quote?.integration}
                                        className="rounded-full w-3 h-3"
                                    />
                                    <p className="capitalize">⚡{transferRoute?.quote?.integration}</p>
                        ...
                        <div className="flex items-center space-x-1 bg-white p-2 rounded-xl">
                            {!transferRoute ? (
                                ...
                                <div className="flex space-x-1 font-bold rounded-full text-[10px] px-2 py-1">
                                    <p className="capitalize">~{transferRoute?.duration} mins 🔥</p>
                                </div>
                            )}
                        </div>
                    </div>
                    <div className="flex justify-between items-center my-0/!">
                        ...
                        <div className="flex items-center space-x-1 bg-white p-2 rounded-xl">
                            {!transferRoute ? (
                                ...
                                <div className="flex space-x-1 font-bold rounded-full text-[10px] px-2 py-1">
                                    <p className="capitalize">{formatUSD(transferRoute.gasUSD)}</p>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            <div className="">
                {isConnected ? (
                    ...
                    >
                        {fromTokenBalance >= transferParams.amount ? 'Start transfer' : 'Insufficient Balance'}
                        {isLoading && <FontAwesomeIcon className="ml-2" icon={faCircleNotch} spin />}
                    </Button>
                ) : (
                    <Button
